...
resource "oci_core_internet_gateway" "mas_ig" {
  compartment_id = var.compartment_id
...
resource "oci_core_nat_gateway" "mas_ng" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.mas_vcn.id
...
resource "oci_core_route_table" "mas_rt_public" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.mas_vcn.id

  display_name = var.rt_public_display_name
  route_rules {
    network_entity_id = oci_core_internet_gateway.mas_ig.id
...
resource "oci_core_route_table" "mas_rt_private" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.mas_vcn.id

  display_name = var.rt_private_display_name
  route_rules {
    network_entity_id = oci_core_nat_gateway.mas_ng.id
    destination       = "0.0.0.0/0"
    destination_type  = "CIDR_BLOCK"
  }
}
...
resource "oci_core_subnet" "mas_snt_lb" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.mas_vcn.id
  ...
  route_table_id = oci_core_route_table.mas_rt_public.id
}

# App subnet
resource "oci_core_subnet" "mas_snt_app" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.mas_vcn.id
  ...
  route_table_id = oci_core_route_table.mas_rt_private.id
}

# Database subnet
resource "oci_core_subnet" "mas_snt_db" {
  compartment_id = var.compartment_id
  ...
  route_table_id = oci_core_route_table.mas_rt_private.id
  security_list_ids = [
    oci_core_vcn.mas_vcn.default_security_list_id,
    oci_core_security_list.mas_sl_db.id
  ]
}